...
export const useProps = (props) => {
  ...
  const exportFile = async (
    ...
    if (item) {
      ...
      const params = {
        url: `service/report/saleByCategory/export/${item?.categoryId}?timeStart=${timeStart}&timeEnd=${timeEnd}`,
        method: 'GET',
      }
      ...
      try {
        ...
        if (response?.data?.codeNumber == 200) {
          await handleFileDownloaed(response?.data?.data, "csv", `report_service_category_${item?.categoryName}`);
        } else {
